...
class AccountRequestApproved(BaseMails):
    action = 'AccountRequestApproved'
    ...
    def __init__(self, details):
        ...
        self.details['reference_number'] = details['reference_number']
    ...
    def notify_to_redington(self):
        ...
        if len(sets) > 0:
            ...
            if len(available_user_ids_in_groups) > 0:
                NfActions.publish(params={
                    ...
                    'message': "New Account request(%s) Approved" % (self.details['reference_number']),
                    'status': "unread",
        ...
        if len(users_emails) > 0:
            data = {
                ...
                'standard_discount': self.details['discount'],
                'reference_number': self.details['reference_number']
            }
    ...
    def notify_to_partner(self):
        ...
        partner_user_details = PartnerUserDetails.objects.filter(partner=self.details['partner'])
        if len(partner_user_details) != 0:
            parter_user_id = list(partner_user_details.values_list('user_id', flat=True))
            NfActions.publish(params={
                'user': self.details['user'],
                'nf_type': "message",
                'recipients': str(parter_user_id[0]),
                'message': "New Account request(%s) Approved" % (self.details['reference_number']),
                'status': 'unread',
                'details_obj': self.details['pending_request']
            })
            ...
            red_user_object = RedUser.objects.filter(id=parter_user_id[0]).first()
            BaseMails.send_mail(subject=self.details['subject'],
